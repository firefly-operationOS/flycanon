...
class TestAddRelation:
    @pytest.mark.asyncio
    async def test_directed_link_persists(self, repositories, relation_service):
    ...
    @pytest.mark.asyncio
    async def test_self_relation_rejected(self, repositories, relation_service):
        await _seed_item(repositories["knowledge"], "a")
    ...
    @pytest.mark.asyncio
    async def test_unknown_from_item_raises(self, repositories, relation_service):
        await _seed_item(repositories["knowledge"], "b")
    ...
    @pytest.mark.asyncio
    async def test_unknown_to_item_raises(self, repositories, relation_service):
        await _seed_item(repositories["knowledge"], "a")
        with pytest.raises(InvalidRelationError):
            await relation_service.add(
                "a",
...
class TestRemoveRelation:
    @pytest.mark.asyncio
    async def test_remove_deletes_row(self, repositories, relation_service):
        await _seed_item(repositories["knowledge"], "a")
        await _seed_item(repositories["knowledge"], "b")
        row = await relation_service.add(
            "a", CreateRelationRequest(to_item_id="b", kind=RelationKind.related)
        )
        await relation_service.remove(row.id)
        out, _ = await relation_service.list_for_item("a")
        assert out == []

    @pytest.mark.asyncio
    async def test_remove_unknown_id_raises(self, relation_service):
        with pytest.raises(RelationNotFoundError):
            await relation_service.remove("does-not-exist")
