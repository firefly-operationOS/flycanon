...
A free function rather than a service: stateless, dependency-free,
called from both the normaliser and the loader registry. Inputs are
``(bytes, declared_media_type, filename)``; the return is the
*normalised* MIME (lowercased, no parameters), used as the routing
key.

The filename hint disambiguates ZIP-based Office formats (DOCX,
XLSX, PPTX, ODT, ODS, ODP all share the ZIP magic bytes) and picks
up text-ish formats whose bytes don't have unique magic.
"""
...
# all ZIP-based Office formats look the same; the ZIP central
# directory listing pins them down, but the filename is a faster hint).
...
def sniff_media_type(
    ...
    *,
    ...
    Returns the canonical lowercased MIME with no parameters. Falls
    back to ``default`` (also normalised) or ``application/octet-stream``.
    """
    if not data:
    ...
    if data.startswith(_OLE_CFB):
        ...
        # too. The filename extension settles the dispute, falling back
        # to the OLE umbrella.
...
def _peek_zip_kind(data: bytes) -> str | None:
    """Inspect the ZIP central directory to disambiguate OOXML / ODF /
    EPUB / plain ZIP. Falls back to ``None`` on any read error -- the
    caller treats the result as a plain ZIP.
    """
    try:
