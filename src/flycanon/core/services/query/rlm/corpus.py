    ...
    async def _list_in_scope(
        ...
        while True:
            ...
            if not batch or offset >= total:
                break
        return [row for row in rows if _matches(row, f)]
...
def _matches(row: SourceRow, f: Filters) -> bool:
    ...
    falls back to when no knowledge linkage exists). ``knowledge_item_ids`` has
    no source-level analogue -- the whole-document corpus is source-keyed, not
    chunk/knowledge-keyed -- so a non-empty value matches nothing.
    """
