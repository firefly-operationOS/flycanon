...
``flycanon_admin``) -- the application code stays unchanged, only
the connection role differs from the API server (which uses the
unprivileged ``flycanon_app`` role and relies on the GUC hook).
"""
